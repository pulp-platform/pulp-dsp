...
def compute_result(result_parameter, inputs, env, fix_point):
    ...
    srcA = inputs['srcA'].value.astype(my_computeType)
    srcB = inputs['srcB'].value.astype(my_computeType)
    num_samples = inputs['num_samples'].value
    result = np.zeros(2*num_samples, dtype=my_type)
    if fix_point is None or fix_point == 0:
        for n in range(0, num_samples):
            result[(2*n)  ] =  srcA[(2*n)  ] * srcB[(2*n)  ] - srcA[(2*n)+1] * srcB[(2*n)+1];    # real part
            result[(2*n)+1] =  srcA[(2*n)  ] * srcB[(2*n)+1] + srcA[(2*n)+1] * srcB[(2*n)  ];    # imag part
    else:
        for n in range(0, num_samples):
            result[(2*n)  ] =  q_sub(q_mul(srcA[(2*n)  ], srcB[(2*n)  ], fix_point, bits=my_bits), q_mul(srcA[(2*n)+1], srcB[(2*n)+1], fix_point, bits=my_bits), bits=my_bits);    # real part
            result[(2*n)+1] =  q_add(q_mul(srcA[(2*n)  ], srcB[(2*n)+1], fix_point, bits=my_bits), q_mul(srcA[(2*n)+1], srcB[(2*n)  ], fix_point, bits=my_bits), bits=my_bits);    # imag part
