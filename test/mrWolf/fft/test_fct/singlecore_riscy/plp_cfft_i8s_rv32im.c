...
/**
  @brief Complex Fourier Transform of 8-bit integer vectors kernel for RV32IM extension
...
void plp_cfft_i8s_rv32im(int8_t *__restrict__ Data, int8_t *__restrict__ Twiddles, uint16_t * __restrict__ SwapTable, uint32_t N_FFT)
{
    uint32_t iLog2N  = 0;
    uint32_t N = N_FFT;
    while(N >>= 1) iLog2N++;
    ...
    for (iCnt1 = 0; iCnt1 < (iLog2N-1); iCnt1++) {
	    ...
	    int8_t W[2] = {CoeffV[iQ], CoeffV[iQ+1]};
            iA = iCnt2 * 2; 
		...
		DataV[iA+1] = (DataV[iA+1] + DataV[iB+1]) /* >> 1 */;
                DataV[iB] = (int8_t)(((int16_t)(Tmp)[0]*(int16_t)(W)[0] - (int16_t)(Tmp)[1]*(int16_t)(W)[1])>>7);
		DataV[iB+1] = (int8_t)(((int16_t)(Tmp)[0]*(int16_t)(W)[1] + (int16_t)(Tmp)[1]*(int16_t)(W)[0])>>7);
                iA = iA + 4 * iM;
            }
            iQ += (iL << 1);
        }
    ...
    for (uint16_t i = 0; i < N_FFT; i++) {
      ...
      if (i < SwapIndex) {
	int8_t Tmp[2] = {DataV[2*i], DataV[2*i+1]};
