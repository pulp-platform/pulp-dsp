...
def gen_stimuli(name, var_type, n_bits, min_value, max_value, length):
    ...
    twiddleCoef_512 = np.empty(2*length//2)
    twiddleCoef_512[0:2*length//2-1:2] = np.floor(np.cos(np.arange(length//2) * 2*np.pi/length)*2**15)
    twiddleCoef_512[1:2*length//2:2] = -np.floor(np.sin(np.arange(length//2) * 2*np.pi/length)*2**15)
    twiddleCoef_512_q16 = np.empty(2*length//2, dtype='int'+str(n_bits))
    twiddleCoef_512_q16 = twiddleCoef_512.astype('int'+str(n_bits))
    ...
    write_arr(f, 'twiddleCoef_512_q16', twiddleCoef_512_q16, var_type, 2*length//2)
...
if __name__=='__main__':
    ...
    gen_stimuli('fft_data16.h', 'int16_t', 16, -2**7, 2**7-1, 512)
