...
def compute_result(result_parameter, inputs, env, fix_point):
    ...
    if result_parameter.ctype == 'int32_t':
        ...
        #complex_result = np.zeros(int(len(a)), dtype=np.csingle)
        ...
        result[1]=(np.real(complex_result[int(len(a)/2)]))

    #elif result_parameter.ctype == 'float':
    #    my_type = np.float32
    #    a = inputs['pSrc'].value.astype(np.float32)
    #    complex_a = np.zeros(int(len(a)), dtype=np.csingle)
    #    #for i in range(len(a)):
    #    #    complex_a[i] = a[i].astype(np.csingle) + (0)*1j
