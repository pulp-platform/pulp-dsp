...
static int t0__do_bench(rt_perf_t *perf, int events, int do_check) {
    ...
    plp_mat_scale_i8(t0__pSrc, t0__blockSize, t0__scaleFactor, t0__shift, t0__pDst);
...
static int t1__do_bench(rt_perf_t *perf, int events, int do_check) {
    ...
    // call the function-under-test
    plp_mat_scale_i8(t1__pSrc, t1__blockSize, t1__scaleFactor, t1__shift, t1__pDst);
...
static int t2__do_bench(rt_perf_t *perf, int events, int do_check) {
    ...
    // call the function-under-test
    plp_mat_scale_i8(t2__pSrc, t2__blockSize, t2__scaleFactor, t2__shift, t2__pDst);
...
static int t3__do_bench(rt_perf_t *perf, int events, int do_check) {
    ...
    // call the function-under-test
    plp_mat_scale_i8(t3__pSrc, t3__blockSize, t3__scaleFactor, t3__shift, t3__pDst);
...
static int t4__do_bench(rt_perf_t *perf, int events, int do_check) {
    ...
    // call the function-under-test
    plp_mat_scale_i8(t4__pSrc, t4__blockSize, t4__scaleFactor, t4__shift, t4__pDst);
...
static void t0__run_test(void) {
    ...
    t0__pSrc = rt_alloc(RT_ALLOC_CL_DATA, sizeof(int8_t) * 1);
    ...
    t0__pDst = rt_alloc(RT_ALLOC_CL_DATA, sizeof(int8_t) * 1);
    ...
    // free up all memory
    rt_free(RT_ALLOC_CL_DATA, t0__pSrc, sizeof(int8_t) * 1);
    rt_free(RT_ALLOC_CL_DATA, t0__pDst, sizeof(int8_t) * 1);
...
static void t1__run_test(void) {
    ...
    t1__pSrc = rt_alloc(RT_ALLOC_CL_DATA, sizeof(int8_t) * 24);
    ...
    t1__pDst = rt_alloc(RT_ALLOC_CL_DATA, sizeof(int8_t) * 24);
    ...
    // free up all memory
    rt_free(RT_ALLOC_CL_DATA, t1__pSrc, sizeof(int8_t) * 24);
    rt_free(RT_ALLOC_CL_DATA, t1__pDst, sizeof(int8_t) * 24);
...
static void t2__run_test(void) {
    ...
    t2__pSrc = rt_alloc(RT_ALLOC_CL_DATA, sizeof(int8_t) * 25);
    ...
    t2__pDst = rt_alloc(RT_ALLOC_CL_DATA, sizeof(int8_t) * 25);
    ...
    // free up all memory
    rt_free(RT_ALLOC_CL_DATA, t2__pSrc, sizeof(int8_t) * 25);
    rt_free(RT_ALLOC_CL_DATA, t2__pDst, sizeof(int8_t) * 25);
...
static void t3__run_test(void) {
    ...
    t3__pSrc = rt_alloc(RT_ALLOC_CL_DATA, sizeof(int8_t) * 26);
    ...
    t3__pDst = rt_alloc(RT_ALLOC_CL_DATA, sizeof(int8_t) * 26);
    ...
    // free up all memory
    rt_free(RT_ALLOC_CL_DATA, t3__pSrc, sizeof(int8_t) * 26);
    rt_free(RT_ALLOC_CL_DATA, t3__pDst, sizeof(int8_t) * 26);
...
static void t4__run_test(void) {
    ...
    t4__pSrc = rt_alloc(RT_ALLOC_CL_DATA, sizeof(int8_t) * 27);
    ...
    t4__pDst = rt_alloc(RT_ALLOC_CL_DATA, sizeof(int8_t) * 27);
    ...
    // free up all memory
    rt_free(RT_ALLOC_CL_DATA, t4__pSrc, sizeof(int8_t) * 27);
    rt_free(RT_ALLOC_CL_DATA, t4__pDst, sizeof(int8_t) * 27);
