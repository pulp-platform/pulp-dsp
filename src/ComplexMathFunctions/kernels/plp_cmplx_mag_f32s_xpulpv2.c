 ...
 * Title:        plp_cmplx_mag_f32s_xpulpv2.c
 * Description:  f32 complex magnitude for XPULPV2
 *
 * $Date:        31. July 2020
 * $Revision:    V0
 *
...
void plp_cmplx_mag_f32s_xpulpv2(const float32_t *pSrc,
                                float32_t *pRes,
                                uint32_t numSamples){
    float32_t real, cmplx, sqr;
    for (int i = 0; i < numSamples; i++) {

        real = *pSrc++;
        cmplx = *pSrc++;
        sqr = (real*real) + (cmplx*cmplx);
        plp_sqrt_f32(&sqr, &pRes[i]);
    }
}
