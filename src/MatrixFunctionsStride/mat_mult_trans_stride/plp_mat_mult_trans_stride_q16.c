...
/**
  @brief Glue code for strided matrix multiplication of 16-bit fix-point matrices.
...
void plp_mat_mult_trans_stride_q16(const int16_t *__restrict__ pSrcA,
    ...
    if (rt_cluster_id() == ARCHI_FC_CID) {
        plp_mat_mult_trans_stride_q16s_rv32im(pSrcA, pSrcB, M, N, O, strideA, strideB, strideC,
                                              shift, pDstC);
    } else {
        plp_mat_mult_trans_stride_q16s_xpulpv2(pSrcA, pSrcB, M, N, O, strideA, strideB, strideC,
                                               shift, pDstC);
    }
}
