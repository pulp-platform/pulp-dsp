 ...
 * Title:        plp_mat_mult_trans_stride_q32s_xpulpv2.c
 * Description:  32-bit fix-point strided matrix multiplication for XPULPV2
 *
 * $Date:        14. July 2020
 * $Revision:    V0
 *
...
#define BASIC_VERSION // if used don't forget to also use the undefine at end of file
    ...
    for (m = 0; m < M; m++) {
        for (o = 0; o < O; o++) {
            int32_t sum = 0;
            for (n = 0; n < N; n++) {
                sum += __ROUNDNORM_REG(pSrcA[m * strideA + n] * pSrcB[o * strideB + n], shift);
            }
