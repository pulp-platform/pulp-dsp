...
/**
  @brief Glue code for strided matrix multiplication of 32-bit fix-point matrices.
...
void plp_mat_mult_trans_stride_q32(const int32_t *__restrict__ pSrcA,
    ...
    if (rt_cluster_id() == ARCHI_FC_CID) {
        plp_mat_mult_trans_stride_q32s_rv32im(pSrcA, pSrcB, M, N, O, strideA, strideB, strideC,
                                              shift, pDstC);
    } else {
        plp_mat_mult_trans_stride_q32s_xpulpv2(pSrcA, pSrcB, M, N, O, strideA, strideB, strideC,
                                               shift, pDstC);
    }
}
