 ...
 * Title:        plp_mat_mult_trans_cmplx_stride_q8vp_xpulpv2.c
 * Description:  parallel 8-bit fix-point complex strided matrix transpose matrix multiplication for
 * XPULPV2
 *
 * $Date:        18. July 2020
 * $Revision:    V0
...
/**
  @brief      parallel strided matrix transpose matrix multiplication for complex 8-bit fix-point on
              XpulpV2
  @param[in]  args    pointer to plp_mat_mat_mult_trans_cmplx_instance_q8 struct initialized by
...
void plp_mat_mult_trans_cmplx_stride_q8vp_xpulpv2(void *args) {
    ...
    plp_mat_mult_cmplx_stride_instance_q8 *a = (plp_mat_mult_cmplx_stride_instance_q8 *)args;
...
#define BASIC_VERSION // if used don't forget to also use the undefine at end of file
    ...
    for (int m = core_id; m < M; m += nPE) {
        for (int o = 0; o < O; o++) {
            ...
            pDstC[(m * strideC + o) * 2 + 0] = (int8_t)sum_re;
            pDstC[(m * strideC + o) * 2 + 1] = (int8_t)sum_im;
        }
    }
