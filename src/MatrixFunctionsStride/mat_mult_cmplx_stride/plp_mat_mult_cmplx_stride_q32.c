 ...
 * Title:        plp_mat_mult_cmplx_stride_q32.c
 * Description:  16-bit fix-point complex strided matrix matrix multiplication glue code
 *
 * $Date:        18. July 2020
 * $Revision:    V0
 *
...
void plp_mat_mult_cmplx_stride_q32(const int32_t *__restrict__ pSrcA,
    ...
    if (rt_cluster_id() == ARCHI_FC_CID) {
        plp_mat_mult_cmplx_stride_q32s_rv32im(pSrcA, pSrcB, M, N, O, strideA, strideB, strideC,
                                              shift, pDstC);
    } else {
        plp_mat_mult_cmplx_stride_q32s_xpulpv2(pSrcA, pSrcB, M, N, O, strideA, strideB, strideC,
                                               shift, pDstC);
    }
}
