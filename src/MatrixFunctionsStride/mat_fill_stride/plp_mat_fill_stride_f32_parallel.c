 ...
 * Title:        plp_mat_fill_stride_f32_parallel.c
 * Description:  parallel 32-bit floating-point strided matrix fill glue code
 *
 * $Date:        17. July 2020
 * $Revision:    V0
 *
...
void plp_mat_fill_stride_f32_parallel(
    ...
    if (hal_cluster_id() == ARCHI_FC_CID) {
        ...
        plp_mat_fill_stride_instance_f32 args = {
            .M = M, .N = N, .stride = stride, .value = value, .nPE = nPE, .pDst = pDst
        };

        hal_cl_team_fork(nPE, plp_mat_fill_stride_f32p_xpulpv2, (void *)&args);
    }
}
