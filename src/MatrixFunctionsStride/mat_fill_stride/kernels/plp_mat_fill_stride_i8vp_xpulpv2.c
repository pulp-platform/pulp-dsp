 ...
 * Title:        plp_mat_fill_stride_i8vp_xpulpv2.c
 * Description:  parallel 8-bit integer strided matrix fill for XPULPV2
 *
 * $Date:        17. July 2020
 * $Revision:    V0
 *
...
/**
  ...
  @param[in]  args  pointer to plp_mat_mat_fill_stride_instance_i8 struct initialized by
...
void plp_mat_fill_stride_i8vp_xpulpv2(void *args) {
    ...
    plp_mat_fill_stride_instance_i8 *a = (plp_mat_fill_stride_instance_i8 *)args;

    uint32_t M = a->M;
    uint32_t N = a->N;
    uint32_t stride = a->stride;
    int8_t value = a->value;
    uint32_t nPE = a->nPE;
