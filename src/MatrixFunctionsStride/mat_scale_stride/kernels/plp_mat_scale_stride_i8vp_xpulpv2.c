 ...
 * Title:        plp_mat_scale_stride_i8vp_xpulpv2.c
 * Description:  parallel 8-bit integer strided matrix scale for XPULPV2
 *
 * $Date:        6. July 2020
 * $Revision:    V0
 *
...
void plp_mat_scale_stride_i8vp_xpulpv2(void *args) {
    ...
    plp_mat_scale_stride_instance_i8 *a = (plp_mat_scale_stride_instance_i8 *)args;
...
#define BASIC_VERSION // if used don't forget to also use the undefine at end of file
    ...
    for (int m = core_id; m < M; m += nPE) {
        for (int n = 0; n < N; n++) {
            ...
            pDst[m * strideDst + n] = (int8_t)(val >> shift);
        }
    }
