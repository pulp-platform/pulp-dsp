 ...
 * Title:        plp_mat_mult_stride_i8.c
 * Description:  8-bit integer strided matrix multiplication glue code
 *
 * $Date:        14. July 2020
 * $Revision:    V0
 *
...
/**
  @brief Glue code for strided matrix multiplication of 8-bit integer matrices.
...
void plp_mat_mult_stride_i8(const int8_t *__restrict__ pSrcA,
    ...
    if (rt_cluster_id() == ARCHI_FC_CID) {
        plp_mat_mult_stride_i8s_rv32im(pSrcA, pSrcB, M, N, O, strideA, strideB, strideC, pDstC);
    } else {
        plp_mat_mult_stride_i8v_xpulpv2(pSrcA, pSrcB, M, N, O, strideA, strideB, strideC, pDstC);
    }
}
