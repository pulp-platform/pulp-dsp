...
/**
  @brief Glue code for strided matrix multiplication of 8-bit fix-point matrices.
...
void plp_mat_mult_stride_q8(const int8_t *__restrict__ pSrcA,
    ...
    if (rt_cluster_id() == ARCHI_FC_CID) {
        plp_mat_mult_stride_q8s_rv32im(pSrcA, pSrcB, M, N, O, strideA, strideB, strideC, shift,
                                       pDstC);
    } else {
        plp_mat_mult_stride_q8s_xpulpv2(pSrcA, pSrcB, M, N, O, strideA, strideB, strideC, shift,
                                        pDstC);
    }
}
