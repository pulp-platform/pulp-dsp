 ...
 * Title:        plp_mat_fill_I_stride_q16vp_xpulpv2.c
 * Description:  parallel 16-bit fix-point strided identity matrix creation for XPULPV2
 *
 * $Date:        17. July 2020
 * $Revision:    V0
 *
...
/**
  ...
  @param[in]  args  pointer to plp_mat_mat_fill_I_stride_instance_q16 struct initialized by
...
void plp_mat_fill_I_stride_q16vp_xpulpv2(void *args) {
    ...
    plp_mat_fill_I_stride_instance_q16 *a = (plp_mat_fill_I_stride_instance_q16 *)args;
...
#define BASIC_VERSION // if used don't forget to also use the undefine at end of file
    ...
    for (int i = core_id; i < N; i += nPE) {
        for (int j = 0; j < N; j++) {
            pDst[i * stride + j] = (int16_t)(i == j) << fracBits;
        }
    }
