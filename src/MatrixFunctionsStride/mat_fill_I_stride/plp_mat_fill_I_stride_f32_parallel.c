 ...
 * Title:        plp_mat_fill_I_stride_f32_parallel.c
 * Description:  parallel 32-bit floating-point strided identity matrix creation glue code
 *
 * $Date:        17. July 2020
 * $Revision:    V0
 *
...
void plp_mat_fill_I_stride_f32_parallel(uint32_t N,
    ...
    if (hal_cluster_id() == ARCHI_FC_CID) {
        ...
        plp_mat_fill_I_stride_instance_f32 args = {
            .N = N, .stride = stride, .nPE = nPE, .pDst = pDst
        };

        hal_cl_team_fork(nPE, plp_mat_fill_I_stride_f32p_xpulpv2, (void *)&args);
    }
}
