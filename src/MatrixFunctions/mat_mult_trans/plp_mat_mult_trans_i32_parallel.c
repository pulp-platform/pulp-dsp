 ...
 * Title:        plp_mat_mult_i32_parallel.c
...
/**
  @brief Glue code for parallel matrix mutliplication of 32-bit integer matrices.
...
void plp_mat_mult_trans_i32_parallel(const int32_t *__restrict__ pSrcA,
    ...
    if (hal_cluster_id() == ARCHI_FC_CID) {
        ...
        plp_mat_mult_instance_i32 args = {
            .pSrcA = pSrcA, .pSrcB = pSrcB, .M = M, .N = N, .O = O, .nPE = nPE, .pDstC = pDstC
        };
        hal_cl_team_fork(nPE, plp_mat_mult_trans_i32p_xpulpv2, (void *)&args);
    }
}
