 ...
 * Title:        plp_mat_mult_trans_cmplx_q16.c
 * Description:  16-bit fix-point complex matrix transpose matrix multiplication glue code
 *
 * $Date:        17. July 2020
 * $Revision:    V0
 *
...
void plp_mat_mult_trans_cmplx_q16(const int16_t *__restrict__ pSrcA,
    ...
    if (rt_cluster_id() == ARCHI_FC_CID) {
        plp_mat_mult_trans_cmplx_q16s_rv32im(pSrcA, pSrcB, M, N, O, shift, pDstC);
    } else {
        plp_mat_mult_trans_cmplx_q16s_xpulpv2(pSrcA, pSrcB, M, N, O, shift, pDstC);
    }
}
