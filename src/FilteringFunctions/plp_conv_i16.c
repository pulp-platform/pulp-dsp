...
void plp_conv_i16(
  ...
  if (rt_cluster_id() == ARCHI_FC_CID){
    if(srcALen >= srcBLen){
      plp_conv_i16s_rv32im(pSrcA, srcALen, pSrcB, srcBLen, pRes);
    } else {
      plp_conv_i16s_rv32im(pSrcB, srcBLen, pSrcA, srcALen, pRes);
    }
  }
  else{
    if(srcALen >= srcBLen){
      plp_conv_i16s_xpulpv2(pSrcA, srcALen, pSrcB, srcBLen, pRes);
    } else {
      plp_conv_i16s_xpulpv2(pSrcB, srcBLen, pSrcA, srcALen, pRes);
    }
  }  
}
