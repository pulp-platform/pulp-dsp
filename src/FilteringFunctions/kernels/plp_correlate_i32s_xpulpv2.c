 ...
 * Title:        plp_correlate_i32s_xpulpv2.c
 * Description:
 *
 * $Date:        03.07.2020
 *
...
void plp_correlate_i32s_xpulpv2(const int32_t *pSrcA,
    ...
    for (int i = 1; i < src2Len; i++) {
        for (int j = 0; j < i; j++) {
            temp += pSrc1[j] * pSrc2[src2Len - i + j];
        }
        *pRes++ = temp;
        temp = 0;
    }
    ...
    for (int i = src2Len - 1; i > 0; i--) {
        for (int j = 0; j < i; j++) {
            temp += pSrc1[offset + src2Len - i + j] * pSrc2[j];
        }
        *pRes++ = temp;
        temp = 0;
    }
